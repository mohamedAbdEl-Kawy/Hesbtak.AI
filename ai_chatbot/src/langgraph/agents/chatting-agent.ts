...
export async function chattingAgentNode(
  ...
  const { userQuery, orgSlug, intent, agentOutput, unresolvedIntent } = state;
...
Please translate and format this agent output into a friendly, professional response to the user. Do not invent any facts not present in the agent output, but present it beautifully.`;
    }

    const response = await groqClient.chat.completions.create({
      model: LLM_MODELS.CHATTING_AGENT,
      messages: [
        { role: 'system', content: `You are Hesbetak.AI, a premium financial assistant chatting for the organization "${orgSlug}".` },
    ...
    let finalResponse = '';
    if (unresolvedIntent) {
      finalResponse = `Hello. I am the financial assistant for ${orgSlug}. I couldn't identify the intent of your query "${userQuery}". Could you please clarify if you are asking about transaction history (database search), cost optimization (financial reasoning), or compiling a report?`;
    } else {
      finalResponse = `Hello. I am the financial assistant for ${orgSlug}. Based on your query "${userQuery}", the specialized agent completed the task: ${agentOutput}. Let me know if you need any additional details!`;
    }
    return { finalResponse };
  }
}
